...
func _apply_looted_state() -> void:
	...
	if main_node == null:
		return
	if barrel_id in main_node.looted_containers:
		set_as_looted()
...
func loot_barrel():
	...
	for i in range(item_count):
		...
		var random_item = main_node.all_items_list.pick_random()
	...
	main_node.looted_containers.append(barrel_id)
	set_as_looted()
...
func _on_barrel_mouse_entered() -> void:
